...
def convnet(model_name, num_class, config=model_config):
    config = config[model_name]
    planes = config["planes"]
    conv_config = config["conv_config"]
    maxpool_cofig = config["maxpool_config"]
    linear_config = config["linear_config"]
    return Convnet(planes, conv_config, maxpool_cofig, linear_config, num_class)

if __name__ == "__main__":
    model = convnet("model6", 6)
